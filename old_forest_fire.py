...
WIND_DIRECTION = 270  # Travelling west
WIND_SPEED = 30       # At 30 MPH (might need to be meters?)
...
def burn_forest(trees, grid_width, grid_height, dist, time):
    x = int(np.floor(grid_width / 2))
    y = int(np.floor(grid_height / 2))

    burning_trees = [
        [x, y]]  # list of indexes of burning trees from list 'trees'. Initially populated with the first burning tree.
    burnt_trees = [
        [0, 0]]  # list of indexes of burning trees from list 'trees'. Initially populated with the first burning tree.
        ...
        for j in range(burning_trees_len):
            ...
            for i in range(-dist, dist + 1):
                for k in range(-dist, dist + 1):
                    x = tree[0] + i
                    y = tree[1] + k
                    if (x < grid_width and y < grid_height and try_ignite(trees[y][x])):
...
# MAIN ##
def main():
    trees = init_grid_forest()
    burn_grid = burn_forest(trees, grid_width, grid_height, dist, time)
    draw_grid(trees)
